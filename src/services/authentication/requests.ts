...
export type TokenResponse = {
  ...
  refresh_token_expires_in: number
        ...
        export const resetPasswordVerify = (body: ResetPasswordVerifyRequestBody): Promise<SuccessResponse> => {
          const { table, ...rest } = body;
          const model = pluralize.singular(table);
          return serviceFetch(
            {
...
email: string;
        ...
        export const signupWithEmail = (body: EmailProviderSignupBodyRequest): Promise<EmailProviderSignupResponse> => {
          const { table, ...rest } = body;
                    return serviceFetch(
            {
        ...
        export const revokeTokenRequest = (body: RevokeTokenRequestBody) => {
        return signIn('revokeToken', {
          token: body.token,
